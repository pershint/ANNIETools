...
def EstimateNeutronEfficiency(Sdf, Bdf, Sdf_trig, Bdf_trig):
    ...
    NeutronProbProfile = np.arange(0,1,0.01)
    ChiSquare = PLBuilder.BuildLikelihoodProfile(NeutronProbProfile,Sbins_normed,Sbins_normed_unc,500000)
...
def EstimateNeutronEfficiencyAllPosns(PositionDict,Bdf,Bdf_trig):
    ...
    for Position in PositionDict:
        ...
        PLBuilder.SetBkgMeanUnc(np.sqrt(pcov[0][0]))
        NeutronProbProfile = np.arange(0,1,0.01)
        ChiSquare = PLBuilder.BuildLikelihoodProfile(NeutronProbProfile,Sbins_normed,Sbins_normed_unc,300000)
        ChiSquare_normed = ChiSquare/np.min(ChiSquare)
